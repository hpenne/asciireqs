...
def main() -> None:
    ...
    reqs = project.requirements.values() if args.recursive else project.root_document.reqs.values()
    path, base = os.path.split(args.reqdoc)
    if args.output_dir:
        path = args.output_dir
    csv_file_name = os.path.join(path, os.path.splitext(base)[0] + '.csv')
    with open(csv_file_name, 'w') as csv_file:
        csv_file.write(','.join(project.root_document.keys))
        csv_file.write('\n')
        for req in reqs:
            csv_file.write(','.join(
                req[key] if key in req else '' for key in project.root_document.keys))
            csv_file.write('\n')
